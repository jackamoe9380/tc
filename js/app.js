...
function slots(theme='crimson7s'){
 const themes={
  crimson7s:{name:'Crimson 7s',className:'crimson-slot',symbols:['7','BAR','★','◆','♠','♥','♣','♦'],pays:{'7':12,'BAR':8,'★':6,'◆':5},tag:'CLASSIC CRIMSON'},
  royalreels:{name:'Royal Velvet',className:'royal-slot',symbols:['♛','♔','♕','A','K','Q','J','10'],pays:{'♛':12,'♔':10,'♕':8,'A':6},tag:'ROYAL HIGH ROLLER'},
  moonlight:{name:'Moonlight Gems',className:'moon-slot',symbols:['◆','✦','●','✧','☾','7','◈','✹'],pays:{'◆':14,'✦':11,'●':8,'✧':7,'☾':6},tag:'MIDNIGHT GEMS'}
 };
 const t=themes[theme]||themes.crimson7s;
 body.innerHTML=`<div class="game-stage slot-stage ${t.className}">
   <div class="slot-topline"><div><span class="eyebrow">${t.tag}</span><h3>${t.name}</h3></div><div class="slot-status"><span></span><b id="slotStatus">READY</b></div></div>
   <div class="slot-cabinet"><div class="slot-lights"></div><div class="reels-window" id="slotReels">
     <div class="slot-reel" id="s1"><div class="slot-reel-strip"></div></div><div class="slot-reel" id="s2"><div class="slot-reel-strip"></div></div><div class="slot-reel" id="s3"><div class="slot-reel-strip"></div></div><div class="payline"></div>
   </div><div class="slot-result" id="slotResult">MATCH 3 FOR THE BIGGEST WIN</div></div>
   <div class="slot-pay-strip">${Object.entries(t.pays).map(([k,v])=>`<span>${k}<b>${v}×</b></span>`).join('')}<span>ANY PAIR<b>2×</b></span></div>
   <div class="bet-bar"><input class="bet-input" id="sbet" type="number" value="100" min="1"><button class="chip" onclick="$('#sbet').value=25">25</button><button class="chip" onclick="$('#sbet').value=100">100</button><button class="chip" onclick="$('#sbet').value=500">500</button><button class="mini-btn red spin-big" id="sspin">SPIN REELS</button></div>
   <div class="message" id="smsg">Set your stake and spin.</div></div>`;
 const rs=[$('#s1'),$('#s2'),$('#s3')]; let spinning=false, timers=[], token=gameSession;
 const pick=()=>t.symbols[Math.floor(Math.random()*t.symbols.length)];
 const symbolMarkup=sym=>{const safe=String(sym);return `<div class="slot-symbol">${safe}</div>`};
 function prepare(reel,final,targetIndex){
   const strip=reel.querySelector('.slot-reel-strip');strip.innerHTML='';
   const total=targetIndex+4;
   for(let i=0;i<total;i++){let sym=(i===targetIndex)?final:pick();strip.insertAdjacentHTML('beforeend',symbolMarkup(sym));}
   strip.style.transition='none';strip.style.transform='translateY(0px)';void strip.offsetWidth;
 }
 function finish(finals,bet){
   if(token!==gameSession||!spinning)return; spinning=false;
   const [a,b,c]=finals;let mult=0;if(a===b&&b===c)mult=t.pays[a]||5;else if(a===b||b===c||a===c)mult=2;
   ...
   if(delta>0){$('#slotResult').classList.add('slot-win');const tm=setTimeout(()=>{if(token===gameSession)$('#slotResult').classList.remove('slot-win')},700);timers.push(tm)}
 }
 function spin(){
   ...
   rs.forEach((r,i)=>{const target=18+i*4;prepare(r,finals[i],target);const strip=r.querySelector('.slot-reel-strip');strip.style.transition=`transform ${1.15+i*.28}s cubic-bezier(.08,.82,.16,1)`;timers.push(setTimeout(()=>{if(token===gameSession&&spinning)strip.style.transform=`translateY(-${target*56}px)`},40+i*120));});
   timers.push(setTimeout(()=>finish(finals,bet),1900));
 }
 $('#sspin').onclick=spin;activeGameCleanup=()=>{timers.forEach(clearTimeout);timers=[];spinning=false};
}
function poker(){
