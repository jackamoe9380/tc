...
function aviaDash(){
 ...
 function loop(ts){if(!active||token!==gameSession)return;if(!last)last=ts;const dt=Math.min(40,ts-last);last=ts;t+=dt;distance+=dt*speed*0.10;$('#aviaWorld').style.transform=`translateX(${-distance}px)`;
   const autoWave=Math.sin(t*.0021*speed)*0.7;const y=altitude+autoWave;$('#plane').style.top=y+'%';$('#plane').style.transform=`translate(-50%,-50%) rotate(${Math.sin(t*.002)*3-2}deg)`;$('#planeShadow').style.top=Math.min(74,y+11)+'%';
   mult=Math.min(40,mult*Math.pow(1.000014,dt*speed));$('#aviaMult').textContent=mult.toFixed(2)+'×';$('#aviaAlt').textContent='ALT '+Math.floor(360-(y-24)*6)+' · AIRBORNE';
   ...
   // Missiles only end the flight if the plane is actually in their lane. They no longer auto-crash at the first missile.
   const missiles=[['r1',1040,'rocket1',47],['r2',1600,'rocket2',32]];
   for(const [key,at,id,my] of missiles){if(!passed[key]&&distance>at){passed[key]=true;const close=Math.abs(y-my)<7;if(close){$('#'+id).classList.add('hit');finish('crash');return}else{$('#'+id).classList.add('hit');$('#aviaMsg').textContent='Missile avoided — keep flying.'}}}
