...
function poker(){
 ...
 function finish(result){if(!active)return;active=false;controls(false);let pay=result==='win'?pot:result==='tie'?Math.floor(pot/2):0;balance+=pay;const delta=record('Texas Hold’em',bet,pay);$('#pokerMsg').textContent=result==='win'?`You win ${money(delta)} profit.`:result==='tie'?`Split pot — ${money(pay)} returned.`:'House wins.';render()}
 function nextStreet(){if(!active)return;if(stage===1)community.push(draw(),draw(),draw());else if(stage===2)community.push(draw());else if(stage===3)community.push(draw());stage++;render();if(stage===4){const cmp=compare(best7([...hole,...community]),best7([...aiHole,...community]));finish(cmp>0?'win':cmp<0?'lose':'tie')}}
 function think(){if(!active)return;$('#pokerMsg').textContent='House is thinking…';controls(false);const t=setTimeout(()=>{if(!active||token!==gameSession)return;const strength=best7([...aiHole,...community])[0]||0;if(stage<4){if(strength>=2&&Math.random()<.8)$('#pokerMsg').textContent='House calls.';else $('#pokerMsg').textContent='House checks.';nextStreet()}else{nextStreet()}},450);timers.push(t)}
 $('#pdeal').onclick=()=>{if(active)return;bet=Number($('#pbet').value);if(!validBet(bet))return;balance-=bet;pot=bet;deck=makeDeck();hole=[draw(),draw()];aiHole=[draw(),draw()];community=[];stage=1;active=true;render();$('#pokerMsg').textContent='Flop incoming. Check, raise, or fold.';controls(true);$('#pcall').disabled=true};
 $('#pcheck').onclick=()=>{if(active)think()};
 $('#pcall').onclick=()=>{if(!active)return;if(balance<bet){toast('Not enough credits to call.');return}balance-=bet;pot+=bet;think()};
 $('#praise').onclick=()=>{if(!active)return;const raise=Math.min(bet,Math.max(25,Math.floor(bet*.5)));if(balance<raise){toast('Not enough credits to raise.');return}balance-=raise;pot+=raise;$('#pokerMsg').textContent=`You raise ${money(raise)}. House is thinking…`;think()};
